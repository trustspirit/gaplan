...
export function CalendarSettings() {
  ...
  const [unitTokenData, setUnitTokenData] = useState<
    Record<string, { enabled: boolean; token: string }>
  >({})
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([calSnap, pubSnap, unitsSnap]) => {
        ...
        if (unitsSnap.exists())
          setUnitTokenData(unitsSnap.data() as Record<string, { enabled: boolean; token: string }>)
      })
  ...
  const handleToggleUnit = async (scopeId: string) => {
    ...
    try {
      ...
      if (next && !token) {
        ...
        batch.set(
          doc(db, 'settings', 'publicUnits'),
          { [scopeId]: { enabled: false, token: token ?? '' } },
          { merge: true },
        )
      }
              ...
              <div key={region.id} className={styles.unitGroup}>
                <div className={styles.unitRow}>
                  ...
                  <button
                    ...
                    disabled={
                      !unitTokenData[region.id]?.enabled || !unitTokenData[region.id]?.token
                    }
                    title={t('common.copyLink')}
                  >
