...
const STATUS_VARIANT: Record<ProjectStatus, 'success' | 'default' | 'warning'> = {
  active: 'default', done: 'success', dropped: 'warning',
}
        ...
        <Card>
          ...
          <CardBody>
            <div className={styles.form}>
              ...
              <Button onClick={handleCreate} loading={creating} disabled={!title.trim()}>{t('project.create')}</Button>
            </div>
          </CardBody>
        </Card>
          ...
          <CardBody>
            ...
            {projects.map(p => (
              <div key={p.id} className={styles.row}>
                <button type="button" className={styles.rowMain} onClick={() => navigate(`/admin/projects/${p.id}`)}>
