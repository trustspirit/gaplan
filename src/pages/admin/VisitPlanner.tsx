...
export function VisitPlanner() {
  ...
  useTopBar({ subtext: t('admin.visitPlanner') })
          ...
          <form className={styles.form} onSubmit={handleCreate}>
            ...
            <div className={styles.section}>
              <p className={styles.sectionLabel}>
                {t('ward.noSundaysAvailable', { defaultValue: '가능 방문 일요일 선택' })}
              </p>
              <p className={styles.sectionHint}>
                {t('ward.sundayOnlyHint', {
                  defaultValue: '금식일을 제외한 일요일만 선택할 수 있습니다.',
                })}
              </p>
            ...
            <div className={styles.presidentSection}>
              <div className={styles.presidentHeader}>
                <span className={styles.presidentLabel}>
                  {t('task.targetPresidents', { defaultValue: '대상 회장' })}
                </span>
                {selectedPresidents.size > 0 && (
                  <Badge variant="default">
                    {t('task.selectedCount', {
                      count: selectedPresidents.size,
                      defaultValue: `${selectedPresidents.size}명 선택됨`,
                    })}
                  </Badge>
                )}
              ...
              <div className={styles.presidentList}>
                {filteredPresidents.length === 0 ? (
                  <p className={styles.noneText}>
                    {filterRegion
                      ? t('task.noPresidentsInRegion', {
                          defaultValue: '해당 지역에 등록된 회장이 없습니다.',
                        })
            ...
            <Textarea
              ...
              placeholder={t('task.notePlaceholder', {
                defaultValue: '회장이 Task를 받을 때 함께 볼 내용을 입력하세요.',
              })}
              rows={3}
            />
            ...
            <Button type="submit" loading={loading} disabled={!isValid}>
              {selectedPresidents.size > 0
                ? t('task.createCount', {
                    count: selectedPresidents.size,
                    defaultValue: `Task ${selectedPresidents.size}건 생성`,
                  })
                : t('task.create')}
            </Button>
          </form>
