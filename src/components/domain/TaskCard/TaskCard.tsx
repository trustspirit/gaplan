...
export function TaskCard({ task, onAction }: TaskCardProps) {
  ...
  return (
    <div
      className={clsx(
        styles.card,
        isUrgent && !isResponded && styles.urgent,
        isResponded && styles.responded,
      )}
    >
      <div className={styles.left}>
        ...
        <div className={styles.labelGroup}>
          ...
          {isResponded && (
            <span className={styles.respondedHint}>{t('task.awaitingConfirmation')}</span>
          )}
        </div>
      </div>
      <div className={styles.right}>
        {!isResponded && (
          <Badge variant={isOverdue ? 'danger' : isUrgent ? 'danger' : 'warning'}>
            {dDayLabel}
          </Badge>
        )}
        {isResponded && !canReopen && <Badge variant="default">{t('common.waiting')}</Badge>}
